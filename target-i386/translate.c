...
static void gen_lea_modrm(DisasContext *s, int modrm, int *reg_ptr, int *offset_ptr)
{
    ...
    if (s->aflag) {
        ...
        case 0:
            if ((base & 7) == 5) {
                base = -1;
                disp = (int32_t)ldl_code_notaint(s->pc);
                s->pc += 4;
                if (CODE64(s) && !havesib) {
            ...
            break;
        case 1:
            disp = (int8_t)ldub_code_notaint(s->pc++);
            break;
        default:
        case 2:
            disp = ldl_code_notaint(s->pc);
            s->pc += 4;
            break;
        }
...
static inline uint32_t insn_get(DisasContext *s, int ot)
{
    ...
    switch(ot) {
    case OT_BYTE:
        ret = ldub_code_notaint(s->pc);
        s->pc++;
        break;
    case OT_WORD:
        ret = lduw_code_notaint(s->pc);
        s->pc += 2;
        break;
    default:
    case OT_LONG:
        ret = ldl_code_notaint(s->pc);
        s->pc += 4;
        break;
    }
    return ret;
...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
 ...
 reswitch:
    ...
    case 0x0f:
        /**************************/
        /* extended op code */
        b = ldub_code_notaint(s->pc++) | 0x100;
        goto reswitch;
    ...
    case 0xc8: /* enter */
        {
            int level;
            val = lduw_code_notaint(s->pc);
            s->pc += 2;
            level = ldub_code_notaint(s->pc++);
...
#ifdef TARGET_X86_64
            if (s->aflag == 2) {
                offset_addr = ldq_code_notaint(s->pc);
                s->pc += 8;
