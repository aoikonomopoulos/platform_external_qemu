...
static inline void
verify_offsets(TranslationBlock *tb, int code_gen_size)
{
    int limit;

    limit = (1 << (8 * sizeof(tb->tb_jmp_offset[0]))) - 1 - sizeof(tb->tb_jmp_offset[0]);

    if (code_gen_size >= limit) {
        fprintf(stderr, "Possible wraparound in tb_jmp_offset: %d generated bytes > %d\n", code_gen_size, limit);
        abort();
    }
}
                              ...
                              int flags, int cflags)
{
    ...
    verify_offsets(tb, code_gen_size);
